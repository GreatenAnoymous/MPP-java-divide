...
public class MultiagentGraphSolverGurobiTime extends GRBCallback{
	...
	public static boolean bDebugInfo = false;
	...
	//	System.out.printf("%d***%d*\n",paths[0].vertexList.size(),startTimeSteps);
...
//		env.set(GRB.IntParam.Threads, 3);
		...
		if(solveRelaxedFirst && extraSteps == 0){
			...
			while(true){
				...
				try{
					model = prepareModel(env, graph, starts, goals, false, false, startTimeSteps, hasTimeLimit ? timeLimit : -1);
...
//			model.setCallback(null);
			...
			if(val == starts.length){
				...
				if(hasTimeLimit && (timeLimit - (System.currentTimeMillis() - startTime)/1000.0) < 0){
					//System.out.printf("%b,%f,%f\n",hasTimeLimit,timeLimit,(System.currentTimeMillis() - startTime)/1000.0);
					...
					return null;
					//System.out.println(" integer model infeasible. Trying T++");
				}
				else{
