...
class PlanResult{
    Vector<Pair<State,Double>> states=new Vector<>();
    Vector<Pair<Action,Double>> actions=new Vector<>();
    double cost;
    double fmin;
...
public class Ecbs{
       ...
       // start.constraints.setSize(initialStates.size());
        ...
        while (!open.isEmpty()) {
            ...
            //    int ss1=newNode.constraints.get(i).vertexConstraints.size()+newNode.constraints.get(i).edgeConstraints.size();
               ...
               // System.out.println(newNode.constraints.get(i));
                //System.exit(0);
    ...
    public Ecbs(Environment environment, float w){
    ...
    public static long[] ECBS_solve(Problem p){
        ...
        Ecbs solver=new Ecbs(test.mapf, (float)1.5);
    ...
    public static void main(String [] args){
        ecbsProblem test=null;
        try{
            Problem px=Problem.createGridProblem(64,64,0.0,100);
