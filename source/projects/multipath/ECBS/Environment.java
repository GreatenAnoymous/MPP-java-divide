...
class Quartet{
    ...
    public Quartet(State state,Action action,Integer cost1,Integer cost2){
        this.state=state;
        this.action=action;
        this.cost1=cost1;
        this.cost2=cost2;
...
class Neighbor{
    ...
    Action action;
    int cost;
    public Neighbor(State state,Action action, int cost){
        this.state=state;
        this.action=action;
        this.cost=cost;
    }
...
class Conflict{
    ...
    public String toString(){
        ...
        return "At time "+time+":Vertex("+x1+","+y1+")";
        else
...
public class Environment{
    ...
    private HashSet<Location> m_obstacles;
    ...
    public Environment(int dimx,int dimy,HashSet<Location> obstacles,Vector<Location> goals){
        this.m_dimx=dimx;
        this.m_dimy=dimy;
    ...
    int focalStateHeuristic(State s, int gScore,Vector<PlanResult> solution) {
        int numConflicts = 0;
    ...
    int focalTransitionHeuristic(State s1a, State s1b, int gScoreS1a, int gScoreS1b,Vector<PlanResult> solution) {
        int numConflicts = 0;
        for (int i = 0; i < solution.size(); ++i) {
            if (i != m_agentIdx && !solution.get(i).states.isEmpty()) {
    ...
    void onExpandLowLevelNode(State s, int fScore,int gScore) {
        m_lowLevelExpanded++;
    }
    ...
    private boolean stateValid(State s) {
        ...
        if(if_valid==true){
            ...
            if(m_obstacles.contains(new Location(s.x, s.y))){
                return false;
            }
